...
var defaultLanguage = {
    ...
    creditCardTypeJCB: "Credit card type JCB (Japan Credit Bureau) is not accepted",
    ...
    phoneInvalid: "Phone is invalid. Please enter a phone number in the format ###-###-####",
...
var languages = [
    defaultLanguage,
    {
        ...
        creditCardNumberInvalid: "El número de la tarjeta de crédito es inválido",
        creditCardTypeJCB: "No se acepta el tipo de tarjeta de crédito JCB (Japan Credit Bureau)",
        creditCardTypeDC: "No se acepta el tipo de tarjeta de crédito Diners Club",
        CVV2Invalid: "CVV2 no es válido",
        creditCardExpirationInvalid: "La fecha de caducidad de la tarjeta de crédito no es válida",
        ...
        phoneInvalid: "El teléfono no es válido. Ingresa un número de teléfono en el formato ### - ### - ####",
        ...
        passwordMismatch: "La contraseña y la contraseña de confirmación deben coincidir"
    },
    {
        type: "French",
        ...
        creditCardTypeJCB: "Le type de carte de crédit JCB (Japan Credit Bureau) n'est pas accepté",
        creditCardTypeDC: "Le type de carte de crédit Diners Club n'est pas accepté",
        CVV2Invalid: "CVV2 est invalide",
        creditCardExpirationInvalid: "La date d'expiration de la carte de crédit n'est pas valide",
        ...
        phoneInvalid: "Le numéro de téléphone est invalide, veuillez entrer un numéro de téléphone au format ### - ### - ####",
        ...
        passwordMismatch: "Le mot de passe et le mot de passe de confirmation doivent correspondre"
    }
]
...
function validateForm(e, returnIsValid, returnErrorObj) {
    ...
    if (!_ValidationRunning) {
        ...
        $(this).closest('form').find('input[type="text"]').each(function (i, item) {
            $(item).val($.trim($(item).val()));
        });

        var registerMapping = false;
        
        if (typeof onFormPreValidation == 'function') {
            var preValidationMessages = onFormPreValidation(e);
            if (preValidationMessages != null && preValidationMessages.length > 0) {
                for (var i = 0; i < preValidationMessages.length; i++) {

                    let preErrMessage = preValidationMessages[i].message ? preValidationMessages[i].message : preValidationMessages[i];
        ...
        let hasNoOrderType = typeof $('input[name=OrderType]:checked').val() === "undefined";
        let orderTypeValue = '';
        ...
        if (hasNoOrderType || orderTypeValue == '' || orderTypeValueLower.indexOf('card') > -1 || (orderTypeValueLower.indexOf('braintree') > -1 || orderTypeValueLower.indexOf('klarna') > -1 || orderTypeValueLower.indexOf('afterpay') > -1 || orderTypeValueLower.indexOf('affirm') > -1)) {
            try {
                trimOrderFields();
                if ($('#CardNumber').length > 0 && ($('#ValidateCardInfo').length == 0 || !$('#ValidateCardInfo').attr('checked')) && (hasNoOrderType || (orderTypeValueLower.indexOf('square') == -1 && orderTypeValueLower.indexOf('afterpay') == -1 && orderTypeValueLower.indexOf('paypalrest') == -1 && orderTypeValueLower.indexOf('affirm') == -1 && orderTypeValueLower.indexOf('paybright') == -1 && orderTypeValueLower.indexOf('klarna') == -1))) {
                    if (isEmpty('CardNumber')) {
                        ...
                        if ($('#CardCvv2').val().length < 3 || isNaN($('#CardCvv2').val()) || isEmpty('CardCvv2') || isInvalidCVV($('#CardCvv2').val()))
                                errorsObj.push({
                    ...
                    if ($('#CardExpirationMonth').length > 0) {
                        ...
                        if (!IsValidCreditCardDate(month, year))
                            errorsObj.push({
                                message:errorMessages.creditCardExpirationInvalid,
                ...
                if ($('#BillingFirstName').length > 0) {
                    if ((typeof (verifyBFirstName) == 'undefined' || verifyBFirstName) && isEmpty("BillingFirstName"))
                        errorsObj.push({
                            message: errorMessages.firstNameRequired,
                            element: $("#BillingFirstName")
                        });

                    if ((typeof (verifyBLastName) == 'undefined' || verifyBLastName) && isEmpty("BillingLastName"))
                        errorsObj.push({
                            message: errorMessages.lastNameRequired,
                            element: $("#BillingLastName")
                        });

                    if ($('#BillingFullName').length != 0 && (typeof (verifyBFullName) == 'undefined' || verifyBFullName) && isEmpty("BillingFullName"))
                        errorsObj.push({
                    ...
                    if ($('#BillingCountry').length > 0) {
                        if (document.getElementById('BillingCountry').value != isSingleBillingCountryValue && isSingleBillingCountry) {
                            document.getElementById('BillingCountry').value = isSingleBillingCountryValue;
                        }
                    }
                }
                ...
                else if ((typeof (verifyBZip) == 'undefined' || verifyBZip) && isEmpty("BillingZip"))
                    errorsObj.push({
                        message: errorMessages.zipRequired,
                        element: $("#BillingZip")
                    });
                ...
                if ($('#ShippingFirstName').length > 0 && ischeckedss()) {
                    if ((typeof (verifySFirstName) == 'undefined' || verifySFirstName) && isEmpty("ShippingFirstName"))
                        errorsObj.push({
                            message: errorMessages.shippingfirstNameRequired,
                            element: $("#ShippingFirstName")
                        });

                    if ((typeof (verifySLastName) == 'undefined' || verifySLastName) && isEmpty("ShippingLastName"))
                        errorsObj.push({
                            message: errorMessages.shippingLastNameRequired,
                            element: $("#ShippingLastName")
                        });

                    if ($('#ShippingFullName').length != 0 && (typeof (verifySFullName) == 'undefined' || verifySFullName) && isEmpty("ShippingFullName"))
                        errorsObj.push({
                ...
                if (!$('#ShippingIsDifferentThanBilling').is(':checked')) {
                    $('[name^=Shipping]').not('[name=ShippingIsDifferentThanBilling]').each(function (index, item) {
                ...
                if (typeof onFormPostValidation == 'function') {
                    ...
                    var postValidationMessages = onFormPostValidation(e, hasErrors);
                    if (postValidationMessages != null && postValidationMessages.length > 0) {
                        for (var k = 0; k < postValidationMessages.length; k++) {

                            let postErrMessage = postValidationMessages[k].message
                ...
                if (typeof doubleOfferWarning === 'function' && errorsObj.length == 0) {
            ...
            try {
                ...
                if (errorsObj.length > 0) {
                    if (returnErrorObj) {
                        ...
                        if (validator) {
                            $.each(finalObj, function (idx, ele) {
                                if (ele.element) {
                                    let jsEle = $(ele.element)[0];
                                    validator.showError(jsEle);
                                    jsEle.setAttribute(
                                        "data-notify-validity",
                                        true
                                    );
                                    jsEle.addEventListener(
                                        "input",
                                        validator.validElement.bind(validator, jsEle)
                                    );
                                    jsEle.addEventListener(
                                        "blur",
                                        validator.validElement.bind(validator, jsEle)
                                    ...
                                    $(ele.element)[0].setCustomValidity("Invalid");
                                }
                            });
                        }
                        return finalObj;
                    ...
                    finalMessages = errorMessages.validationHeader + "\n" + finalMessages;
                    _ValidationRunning = false;         

                    if (!registerMapping) {
                        registerMapping = true;
                        registerEvent("handleErrorValidationMapping", function (e) {
                            let useAdaErrors = typeof _AdaErrors === "undefined" || !_AdaErrors ? false : _AdaErrors;
                            if (useAdaErrors) {
                                ...
                                for (var i = 0; i < errors.length; i++) {
                                    ...
                                    for (var j = 0; j < errorsLi.length; j++) {
                                        ...
                                        if (message == vseMessage) {
                                            var elementExists = typeof $(element) != "undefined" && $(element).length > 0;
                    ...
                    var validateAddress = function () {
                        if (typeof (QASValidate) != 'undefined' && QASValidate) {
                            e.preventDefault();
                            qasValidateAddress();
                            return true;
                        } else {
                    ...
                    if ((typeof (EmailValidate) == 'undefined' || EmailValidate)) {
                        e.preventDefault();
                        $.post('/shared/services/validate.ashx?at=Email', { value: emailaddress }, function (result) {
                            _ValidationRunning = false;
                            if (result.success) {
                                if (!validateAddress()) {
                                    onEmailValidateComplete();
                                }
                            } else {
                                $.fancybox.open({
                                    type: 'html',
                                    content: $("<textarea/>").html(result.message).text(),
                                    modal: true
                                });
                            }
                        }, 'json')
                            .error(function () {
                                // if service failing skip step
                                _ValidationRunning = false;
                                validateAddress();
                            });
                    } else {
...
function isInvalidCVV(cvv) {
    var hasCardType = $('#CardType').length > 0 && $('#CardType').val().length > 0;
    ...
    if ((typeof $('input[name=OrderType]:checked').val() === "undefined") || $('input[name=OrderType]:checked').val() == '') {
        ...
        try {
            ...
            if (isContinueCCCheck && $('#CardNumber').length > 0 && ($('#ValidateCardInfo').length == 0 || !$('#ValidateCardInfo').attr('checked'))) {
                ...
                if (isEmpty('CardNumber')) {
                    messages += "\t" + errorMessages.creditCardNumberRequired + "\n";
                }
                else {
                    switch (CheckCardNum($('#CardNumber').val())) {
                        case 0:
                            messages += "\t" + errorMessages.creditCardNumberInvalid + "\n";
                            break;

                        case 2:
                            messages += "\t" + errorMessages.creditCardTypeJCB + "\n";
                            break;

                        case 3:
                            messages += "\t" + errorMessages.creditCardTypeDC + "\n";
                            break;
                    }
                }
                if ($('#CardExpirationMonth').length > 0) {
                    ...
                    if (!IsValidCreditCardDate(month, year))
                        messages += "\t" + errorMessages.creditCardExpirationInvalid + "\n";
                }
            }
            ...
            if (typeof (isLoggedIn) != 'undefined' && !isLoggedIn && ($("input[name='LoginSelection']:checked").val() == undefined || $("input[name='LoginSelection']:checked").val() != "anonymous")) {
                ...
                if (isEmpty("ConfirmPassword"))
                    messages += "\t" + errorMessages.confirmPasswordRequired + "\n";
            ...
            if (ischeckedss()) {
                ...
                if (document.getElementById('ShippingStreet').value.length == 0)
                    messages += "\t" + errorMessages.shippingAddressRequired + "\n";

                if (document.getElementById('ShippingCity').value.length == 0)
                    messages += "\t" + errorMessages.shippingCityPlaceholder + "\n";

                if (document.getElementById('ShippingState').value.length == 0)
                    messages += "\t" + errorMessages.shippingStateRequired + "\n";
                ...
                if (document.getElementById('ShippingCountry').value.length == 0)
                    messages += "\t" + errorMessages.shippingCountryRequired + "\n";

                if (document.getElementById('ShippingCountry').value != isSingleShippingCountryValue && isSingleShippingCountry)
                    document.getElementById('ShippingCountry').value = isSingleShippingCountryValue;
...
//    $.post('/shared/js/checkFingerprintJSSetting.aspx', {}, function (r) {
//        if (r.v) {
//            // set up call for fingerprintJS and post of resulting data
//            var $script = $('<script type="text/javascript" src="/shared/fingerprint/fingerprint.min.js"></script>'
//                + '<script>const fpPromise = FingerprintJS.load();'
//                + 'fpPromise.then(agent => agent.get()).then(result =>'
//                + '{ result.components.canvas = "";'
//                + '$.post("/shared/services/savefingerprint.ashx?fpId=" + result.visitorId,'
//                + '{ components: encodeURIComponent(JSON.stringify(result.components)) });'
//                + '});</script > ');
...
$(document).ready(function () {
    ...
    if ($('#BillingCountry').length != 0 && $('#BillingCountry option').length == length) {
        var billingValue = $('#BillingCountry option:last').val();
        var billingParent = $('#BillingCountry').parent();
        billingParent.hide();
        billingParent.closest('form').append('<input type="hidden" id="BillingCountry" name="BillingCountry" value="' + billingValue + '"/>');
    ...
    if ($('#ShippingCountry').length != 0 && $('#ShippingCountry option').length == length) {
        var shippingValue = $('#ShippingCountry option:last').val();
        var shippingParent = $('#ShippingCountry').parent();
        shippingParent.closest('form').append('<input type="hidden" id="ShippingCountry" name="ShippingCountry" value="' + shippingValue + '"/>');
        shippingParent.hide();
...
function getFacebox() {
    // check if facebox js file doesn't exist
    if ($('script[src*="facebox"]').length === 0) {
        // set up js for facebox and load it after jquery is loaded
        var $script = $('<script id="faceboxJS" defer type="text/javascript" src="/shared/facebox/facebox.js"></script>');
        $script.appendTo($('body'));
    }

    // check if facebox css file doesn't exist
    if ($('link[href*="facebox"]').length === 0) {
        // set up css for facebox and append it at the end of the body tag
        var $link = $('<link id="faceboxCSS" type="text/css" rel="stylesheet" href="/shared/facebox/facebox.css">');
        $link.appendTo($('body'));
    }
}

$(window).ready(function () {
    // add support for facebox
    if (typeof loadFacebox == "undefined") {
        getFacebox();
    }
    ...
    try {
        // get data from PO service
        $.post('/shared/services/po.ashx', function (data) {
            if (data.length > 0) {
                // add above shipping address field
                $('<div id="txt_poBox" class="inform-message-container u-vw--100">' + data + '</div>').insertBefore($('#ShippingIsSame'));
            }
        });
    } catch (e) {
    ...
    if (typeof loadFWSnippets == "undefined") {
        GetFWSnippets();
    }
    ...
    setTimeout(function () {
        $('a:not([id])').each(function (index, item) {
            $(item).attr('data-track', 'dtm-link' + index);
        });
        $('body').on('click', 'a', function () {
            var id = this.id == '' ? $(this).attr('data-track') : this.id;
            $.post('/shared/services/pixelservice.ashx?at=click&id=' + encodeURIComponent(id),
                { id: encodeURIComponent(id), url: encodeURIComponent(this.href) });
        });
    }, 1000);
    ...
    try {
        function verifyCookie(cname) {
            ...
            for (var i = 0; i < ca.length; i++) {
                ...
                if (c.indexOf(name) == 0) {
                    results[results.length] = c.substring(name.length, c.length);
                }
            }
            if (results.length > 1) {
                var removeItem = function (sKey, sPath, sDomain) {
                    console.log('More than 1 cookie detected, deleteing: ', sKey, sPath, sDomain);
    ...
    var populateOptionValues = function (existingSelects) {
        var selects = existingSelects || document.getElementsByTagName('select');
        for (var i = 0; i < selects.length; i++) {
            var select = selects[i];
            for (var j = 0; j < select.options.length; j++) {
                var option = select.options[j];
                if (!option.getAttribute('value')) {
                    if ((option.innerText || '').toLowerCase().indexOf('choose') >= 0
                        || (option.innerText || '').toLowerCase().indexOf('estado') >= 0
                        || (option.innerText || '').toLowerCase().indexOf('choisi') >= 0
                        || (option.innerText || '').toLowerCase().indexOf('select') >= 0
                        || (option.innerText || '').toLowerCase().indexOf('out of stock') >= 0) {
    ...
    $(document).on('DOMNodeInsertedIntoDocument DOMNodeInserted', function (e) {
        ...
        if ($('#CardNumber').length > 0 && $('#CardNumber').attr('autocomplete') !== 'cc-number') {
...
registerEvent("SessionLoaded", function () {
    $.get('/shared/services/visitormediatrackingservice.ashx' + window.location.search);
    $.post('/shared/services/pixelservice.ashx?at=log',
        { message: encodeURIComponent("SessionLoadedEventFired") });
    var aIframe = document.createElement('iframe');
    aIframe.setAttribute('src', '/shared/activity.aspx?uref=' + encodeURIComponent(window.location.href));
    aIframe.setAttribute('height', '0');
    aIframe.setAttribute('width', '0');
    aIframe.setAttribute('style', 'display: none; height: 0; width: 0;');
    document.body.appendChild(aIframe);
});

function GetFWSnippets() {

    try {
        // get framework snippets
        $.post('/shared/services/framework.ashx?t=context', function (data) {
            // create framework js
            var s = document.createElement('script');
            s.id = 'DTMFWJS2';
            ...
            if ($('link[href*="framework.css"]').length === 0) {
                // create framework css
                var l = document.createElement('link');
                l.id = 'DTMFWCSS2';
                l.href = data.fwcss;
                l.type = 'text/css';
                l.rel = 'stylesheet';
                // insert framework css into the DOM
                $('head').append(l);
            }
