...
const im = new InstagramMediaManager("17841400288346220");

im.requestMedia().then((media) => console.log(media));
...
const createInstagramGallery = (gallery: Element | null) => {
    if (!gallery) return;
...
const handleCarouselImages = (carousel: Carousel, record: Element): void => {
    ...
    carousel.on("rotation", (currentIndex: number | undefined) => {
...
observer("[data-media-player-src-iframe]", {
    inRange: (record) => {
        new LoadItem(record, {
            ...
            new MediaPlayer(new VimeoMediaPlayerAdapter(player));
        });
    }
});
