...
<%  //Columns to show
    ...
    var propHideRow = product.PropertyIndexer["HideProductOnConfirmation", false];
...
<tr data-count="<%=count %>" style="<%=hideRow%>">
    ...
    <%if (showSubtotalColumn)
        { %>
    <td align="right" data-label="<%= subTotalColumnLabel %>" class="orderItemsTable__body-subtotal"><%=string.Format(new CultureInfo(culture), "{0:C}", item.Price * qty)%></td>
    <%}%>
